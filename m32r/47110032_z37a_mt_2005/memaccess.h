...
#define _readu16(offset)                                                                                               \
	({                                                                                                                 \
		unsigned read_var;                                                                                             \
		__asm__ __volatile__("lduh %0,@(" #offset ",fp)" : "=&r"(read_var) : : "memory");                              \
		read_var;                                                                                                      \
	})
#define _writeu16(val, offset)                                                                                         \
	do {                                                                                                               \
		unsigned write_var = val;                                                                                      \
		__asm__ __volatile__("sth %0,@(" #offset ",fp)" : : "r"(write_var) : "memory");                                \
	} while (0)
#define _read_low_ofu16(offset)                                                                                        \
	({                                                                                                                 \
		unsigned read_var;                                                                                             \
		__asm__ __volatile__("ldub %0,@(" #offset ",fp)" : "=&r"(read_var) : : "memory");                              \
		read_var;                                                                                                      \
	})
...
#define _readu8(offset)                                                                                                \
	({                                                                                                                 \
		unsigned read_var;                                                                                             \
		__asm__ __volatile__("ldub %0,@(" #offset ",fp)" : "=&r"(read_var) : : "memory");                              \
		read_var;                                                                                                      \
	})
#define _writeu8(val, offset)                                                                                          \
	do {                                                                                                               \
		unsigned write_var = val;                                                                                      \
		__asm__ __volatile__("stb %0,@(" #offset ",fp)" : : "r"(write_var) : "memory");                                \
	} while (0)
// turn into fp,#offset later
...
#define HYSTERESIS_FLAG(flag_var, _bit_mask, condition_var, off_value, on_value)                                       \
	do {                                                                                                               \
		...
		if (var & bit_mask) {                                                                                          \
			if (off_value >= covar)                                                                                    \
				writeu16(var & ~bit_mask, flag_var);                                                                   \
		} else {                                                                                                       \
			if (on_value < covar)                                                                                      \
				writeu16(var | bit_mask, flag_var);                                                                    \
		}                                                                                                              \
...
#define BIT_CONDITION16(flag_var, bit_mask, condition)                                                                 \
	do {                                                                                                               \
		if (condition) {                                                                                               \
			BIT_SET16(flag_var, bit_mask);                                                                             \
		} else {                                                                                                       \
			BIT_CLEAR16(flag_var, bit_mask);                                                                           \
		}                                                                                                              \
	} while (0)
...
#define MUT_INJECTION_MARKER_LOWU16(name)                                                                              \
	const unsigned __attribute__((section("mutinj_" #name))) flash_##name##_injected_instruction =                     \
	    (unsigned)(0x808001 + name);
...
#define S_DEC_U16(x)                                                                                                   \
	do {                                                                                                               \
		uint_fast16_t val = readu16(x);                                                                                \
		if (val)                                                                                                       \
			writeu16(val - 1, x);                                                                                      \
	} while (0)

#define S_DEC_DOWNTO_U16(x, boundary)                                                                                  \
	do {                                                                                                               \
		uint_fast16_t val = readu16(x);                                                                                \
		if (val > boundary)                                                                                            \
			writeu16(val - 1, x);                                                                                      \
...
#define S_INC_UPTO_U16(x, boundary)                                                                                    \
	do {                                                                                                               \
		uint_fast16_t val = readu16(x);                                                                                \
		if (val < boundary)                                                                                            \
			writeu16(val + 1, x);                                                                                      \
