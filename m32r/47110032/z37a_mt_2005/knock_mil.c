...
ROM_SECTION("knock_mil_condition_section") unsigned knock_mil_condition()
{
	...
	if ((readu16(map_engine_load) < flash_knock_mil_load_min) && (readu16(tps_main_rcord) < flash_knock_mil_tps_min)) {
		if (!readu16(decays_x1_knock_mil_flash))
			writeu16(0, knock_mil_state);
	} else if (!readu16(decays_x1_knock_mil_flash) || l_reset) {
		unsigned state;
...
ROM_SECTION("update_mil_section") void update_mil()
{
	...
	if (IS_BIT_SET16(sio0_protocol_flags, 0xc3) && IS_BIT_SET16(sio0_state, 0x80))
	...
	if (!readu16(decays_x1_ignition)) {
		...
		for (i = 0; i < 16; ++i) {
			uint16_t *ptr = (uint16_t *)TO_FULL_POINTER(dtc_mil_active_errors);
			if (!ptr[i])
				continue;
			l_mil_on = 1;
			if (IS_BIT_SET16(mode_flags2, 0x2000))
				r10 = 1;
			break;
		}
	}
	...
	if (IS_BIT_SET16(mode_flags2, 0x2000) && r10) {
		BIT_SET16(fp12850_f16, 0x0800);
	} else {
		BIT_CLEAR16(fp12850_f16, 0x0800);
	}

	if (!l_is_flashing && !readu16(knock_mil_state)) {
		writeu16(0, decays_x1_mil_flash_timer);
		writeu16(0, fp11872_u16);
		writeu16(0x0600, fp11870_u16);
	}
}
