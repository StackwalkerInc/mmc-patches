...
{
	if (age_x1_ignition
	    < flash_asc_lamp_startup_time) { // 1 second on at ignition on to test the lamp, then start ceremony takes over
		asc_change_lamp = 1;
...
const uint8_t flash_gear_to_cvt_dash_data[8] = {0x00, 0x20, 0x40, 0x60, 0x80, 0xa0, 0xc0, 0xff};
...
void update_cvt_dash_data()
{
	...
	if (decays_x1_alt_mode_change_cel_flash_timer) {
		uint_fast16_t l_timer =
		    s_divu16(flash_alt_mode_change_cel_flash_time - decays_x1_alt_mode_change_cel_flash_timer,
		...
		if ((torque_request_flags & TORQUE_REQUEST_FLAGS_ASC_INTERVENTION)
		    && flash_asc_flash_pattern[dash_flash_runner]) {
			cvt_dash_data1 = 0;
		}
	}
}
