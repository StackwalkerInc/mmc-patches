...
DECLARE_VALUE_DESC(flash_init_nlts_active_decay_partial_engagement, "NLTS",
                   "Partial Clutch Engagement Active Time Limit", "Time");
...
DECLARE_VALUE_DESC(flash_nlts_rpm_delta_threshold, "NLTS", "Clutch Engagement Detection - Negative Engine RPM Delta",
                   "RPM");

const uint16_t flash_nlts_app_thresholds[2] = {250, 300};
DECLARE_ARRAY_DESC(flash_nlts_app_thresholds, "NLTS", "APPS Threshold", "uint16",
...
DECLARE_AXIS_WITH_DESC(flash_nlts_rpm_axis, NLTS_RPM_AXIS_SIZE, "RPM", "RPM") = {
    TEMP_AXIS_SRC_XDST, .size = NLTS_RPM_AXIS_SIZE, .data = {0x100, 0x200, 0x300, 0x400, 0x500, 0x600, 0x700}};

#define NLTS_APPS_AXIS_SIZE 5

DECLARE_AXIS_WITH_DESC(flash_nlts_apps_axis, NLTS_APPS_AXIS_SIZE, "TPS[0.5-4.5V]", "VoltsADC1023") = {
    TEMP_AXIS_SRC_YDST, .size = NLTS_APPS_AXIS_SIZE, .data = {250, 400, 500, 600, 750}};

DECLARE_3DMAP8_WITH_DESC(flash_nlts_fuelcut_holdoff_3dmap8, NLTS_RPM_AXIS_SIZE, NLTS_APPS_AXIS_SIZE, "NLTS",
                         "Clutch Disengagement Fuel Cut Holdoff Time", "Time40HzU8", "DDflash_nlts_apps_axis",
                         "DDflash_nlts_rpm_axis") = {
    TEMP_3DMAP_SRC, .xsize = NLTS_RPM_AXIS_SIZE,
    .data = {6, 4, 4, 3, 3, 0, 0, 3, 3, 3, 3, 1, 0, 0, 3, 3, 2, 1, 1, 0, 0}};
