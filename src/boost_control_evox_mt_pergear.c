...
DECLARE_AXIS_WITH_DESC(flash_boost_control_gear_axis, GEAR_AXIS_SIZE, "Gear",
                       "uint16") = {TEMP_AXIS_SRC_YDST, .size = GEAR_AXIS_SIZE, .data = {0, 1, 2, 3, 4, 5}};
...
#define DECLARE_BOOST_TARGET_LOAD_MAP(symbol, username)                                                                \
	DECLARE_3DMAP8_WITH_DESC(symbol, BOOST_CONTROL_RPM_AXIS_SIZE, GEAR_AXIS_SIZE, "MT Rcus Moded Boost", username,     \
	                         "Load8", "DDflash_boost_control_gear_axis", "DXflash_boost_control_rpm_axis") = {         \
	    TEMP_3DMAP_SRC, .xsize = BOOST_CONTROL_RPM_AXIS_SIZE, .data = {80}}

#define DECLARE_COEFFICIENT_MAP(symbol, username)                                                                      \
	DECLARE_3DMAP8_WITH_DESC(symbol, BOOST_CONTROL_RPM_AXIS_SIZE, BOOST_CONTROL_TPS_AXIS_SIZE, "MT Rcus Moded Boost",  \
...
#define DECLARE_WGDC_MAP(symbol, username)                                                                             \
	DECLARE_3DMAP8_WITH_DESC(symbol, BOOST_CONTROL_RPM_AXIS_SIZE, GEAR_AXIS_SIZE, "MT Rcus Moded Boost", username,     \
	                         "WGDuty", "DDflash_boost_control_gear_axis", "DXflash_boost_control_rpm_axis") = {        \
...
DECLARE_2DMAP8_WITH_DESC(flash_wastegate_duty_compensation_vs_iat_2dmap8, BOOST_CONTROL_IAT_AXIS_SIZE,
                         "MT Rcus Moded Boost", "Base WGDC vs IAT Correction", "OffsetWGDC",
