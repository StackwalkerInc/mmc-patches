...
DECLARE_AXIS_WITH_DESC(flash_map_compensation_mat_axis, MAT_AXIS_SIZE, "MAT",
                       "Temp") = {TEMP_AXIS_SRC_XDST, .size = MAT_AXIS_SIZE, .data = {0, 8, 33, 49, 63, 78, 96, 125}};

DECLARE_2DMAP8_WITH_DESC(flash_map_load_mat_compensation_2dmap8, MAT_AXIS_SIZE, "MAF Limits with MAP",
                         "MAP Load - MAT Compensation", "Percent128U8",
                         "DDflash_map_compensation_mat_axis") = {TEMP_2DMAP_SRC, .data = {128, 128, 128, 128,

                                                                                          128, 128, 128, 128}};

#define MAP_LIMITS_BARO_AXIS_SIZE 5

DECLARE_AXIS_WITH_DESC(flash_map_compensation_baro_axis, MAP_LIMITS_BARO_AXIS_SIZE, "Barometric Pressure", "Baro") = {
    TEMP_AXIS_SRC_XDST, .size = MAP_LIMITS_BARO_AXIS_SIZE, .data = {0x5d, 0x7d, 0x9d, 0xad, 0xbd}};

DECLARE_2DMAP8_WITH_DESC(flash_map_load_baro_compensation_2dmap8, MAP_LIMITS_BARO_AXIS_SIZE, "MAF Limits with MAP",
                         "MAP Load - Baro Compensation", "Percent128U8",
                         "DDflash_map_compensation_baro_axis") = {TEMP_2DMAP_SRC, .data = {128, 128, 128, 128, 128}};

#define MAP_LIMITS_RPM_AXIS_SIZE 16

DECLARE_AXIS_WITH_DESC(flash_map_engine_rpm_axis, MAP_LIMITS_RPM_AXIS_SIZE, "RPM",
                       "RPM") = {.dst = MAP_RPM_AXIS_DST,
                                 .src = MAP_RPM_AXIS_SRC,
                                 .size = MAP_LIMITS_RPM_AXIS_SIZE,
                                 .data = {0x000, 0x080, 0x100, 0x180,
...
DECLARE_AXIS_WITH_DESC(flash_map_avg_axis, MAP_LIMITS_MAP_AXIS_SIZE, "Manifold Pressure",
                       "MAPModPressure") = {.dst = MAP_AXIS_DST_SDA,
                                            .src = MAP_AXIS_SRC_SDA,
                                            .size = MAP_LIMITS_MAP_AXIS_SIZE,
                                            .data = {0,     1638,  3277,  4915,  6554,

                                                     8192,  9830,  11469, 13107, 14746,

                                                     16384, 18022, 19661, 21299, 22938,

                                                     24576, 26214, 27853, 29491, 31130,

                                                     32768, 34406, 36045, 37683, 39322,
...
static uint32_t update_load_source_value_from_map_samples()
{
	...
	if (l_samples) {
		...
		map_avg16 =
		    s_scale_base256(s_sub16(map_avg_voltage, flash_map_voltage_zero_offset), flash_map_scaling_kpa_per_volt);
	}
